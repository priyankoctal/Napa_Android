...
import com.octalsoftaware.archi.databinding.ProceduralBinding;
...
public class ProceduralActivity extends BaseActivity implements View.OnClickListener {

    @Nullable
    private ProceduralBinding mBinding = null;
    @Nullable
    private Context context = null;
    @Nullable
    private MyProgressDialog progressDialog = null;
    public static String TAG = ProceduralActivity.class.getSimpleName();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.procedural);
        patient_id = getIntent().getStringExtra(S.patient_details);
        database = new Database(context);
        // set Title
        mBinding.mainToolbar.toolbarheading.setText(getString(R.string.procedural));
    ...
    public void onBackPressed() {
        if (Util.isNetworkConnected(context)) {
            try {
                ...
                if (mBinding.checkboxLeft1.isChecked())
                    jsonObject.put(getString(R.string.vascular_access_complication), getString(R.string.intra_op));
                else if (mBinding.checkboxRight1.isChecked())
                    jsonObject.put(getString(R.string.vascular_access_complication), getString(R.string.post_op));
                else
                    jsonObject.put(getString(R.string.vascular_access_complication), getString(R.string.no));

                if (mBinding.checkboxLeft2.isChecked())
                    jsonObject.put(getString(R.string.pneumothorax), getString(R.string.intra_op));
                else if (mBinding.checkboxRight2.isChecked())
                    jsonObject.put(getString(R.string.pneumothorax), getString(R.string.post_op));
                else
                    jsonObject.put(getString(R.string.pneumothorax), getString(R.string.no));
    ...
    private void fillQIDetails(@NonNull CustomArrayList<AdvancedQIModal> save_qi) {
        if (save_qi.contains(getString(R.string.vascular_access_complication).toLowerCase())) {
            int pos = save_qi.indexOf(getString(R.string.vascular_access_complication).toLowerCase());
        ...
        if (save_qi.contains(getString(R.string.pneumothorax).toLowerCase())) {
            int pos = save_qi.indexOf(getString(R.string.pneumothorax).toLowerCase());
            if (save_qi.get(pos).getId().equalsIgnoreCase(getString(R.string.intra_op)))
