...
import com.octalsoftaware.archi.databinding.MorbidityMortalityBinding;
...
public class MorbidityMortality extends BaseActivity implements View.OnClickListener {

    @Nullable
    private MorbidityMortalityBinding mBinding = null;
    @Nullable
    private Context context = null;
    @Nullable
    private MyProgressDialog progressDialog = null;
    public static String TAG = MorbidityMortality.class.getSimpleName();
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.morbidity_mortality);

        patient_id = getIntent().getStringExtra(S.patient_details);
        database = new Database(context);

        // set Title
        mBinding.mainToolbar.toolbarheading.setText(getString(R.string.morbidity_mortality));
    ...
    public void onBackPressed() {
        if (Util.isNetworkConnected(context)) {
            try {
                ...
                if (mBinding.checkboxLeft1.isChecked())
                    jsonObject.put(getString(R.string.death), getString(R.string.intra_op));
                else if (mBinding.checkboxRight1.isChecked())
                    jsonObject.put(getString(R.string.death), getString(R.string.post_op));
                else
                    jsonObject.put(getString(R.string.death), getString(R.string.no));
    ...
    private void fillQIDetails(@NonNull CustomArrayList<AdvancedQIModal> save_qi) {
        if (save_qi.contains(getString(R.string.death).toLowerCase())) {
            int pos = save_qi.indexOf(getString(R.string.death).toLowerCase());
