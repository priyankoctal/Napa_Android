...
import com.octalsoftaware.archi.databinding.UpperExtremityBinding;
...
public class UpperExtremityActivity extends BaseActivity implements View.OnClickListener {

    private String patient_id = "";
    @Nullable
    UpperExtremityBinding mBinding = null;
    @Nullable
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.upper_extremity);

        database = new Database(context);

        // set Title
        mBinding.mainToolbar.toolbarheading.setText(getString(R.string.upper_extremity));
        ...
        if (Util.isNetworkConnected(context)) {
            try {
                ...
                if (mBinding.checkboxFemoral.isChecked())
                    jsonObject.put(S.brachial_plexus, getString(R.string.yes));
                else
                    jsonObject.put(S.brachial_plexus, getString(R.string.no));
                if (mBinding.checkboxAdductor.isChecked())
                    jsonObject.put(S.supraclavicular, getString(R.string.yes));
                else
                    jsonObject.put(S.supraclavicular, getString(R.string.no));
                if (mBinding.checkboxPlexus.isChecked())
                    jsonObject.put(S.interscalene, getString(R.string.yes));
                else
                    jsonObject.put(S.interscalene, getString(R.string.no));

                if (mBinding.checkboxSciatic.isChecked())
                    jsonObject.put(S.axillary, getString(R.string.yes));
                else
                    jsonObject.put(S.axillary, getString(R.string.no));
    ...
    private void autofillInvasiveLine(@NonNull CustomArrayList<AdvancedQIModal> arrayList) {
        if (arrayList.contains(S.brachial_plexus.toLowerCase())) {
            int pos = arrayList.indexOf(S.brachial_plexus.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
                mBinding.checkboxFemoral.setChecked(true);
        }
        if (arrayList.contains(S.supraclavicular.toLowerCase())) {
            int pos = arrayList.indexOf(S.supraclavicular.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
                mBinding.checkboxAdductor.setChecked(true);
        }
        if (arrayList.contains(S.interscalene.toLowerCase())) {
            int pos = arrayList.indexOf(S.interscalene.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
                mBinding.checkboxPlexus.setChecked(true);
        }
        if (arrayList.contains(S.axillary.toLowerCase())) {
            int pos = arrayList.indexOf(S.axillary.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
