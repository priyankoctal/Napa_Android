...
import com.octalsoftaware.archi.databinding.NeurxialBinding;
...
public class NEURAXIAL extends BaseActivity implements View.OnClickListener {

    private String patient_id = "";
    @Nullable
    NeurxialBinding mBinding = null;
    @Nullable
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mBinding = DataBindingUtil.setContentView(this, R.layout.neurxial);

        database = new Database(context);

        // set Title
        mBinding.mainToolbar.toolbarheading.setText(getString(R.string.neuraxial));
        ...
        if (Util.isNetworkConnected(context)) {
            try {
                ...
                if (mBinding.checkboxFemoral.isChecked())
                    jsonObject.put(S.thoracic_epidural, getString(R.string.yes));
                else
                    jsonObject.put(S.thoracic_epidural, getString(R.string.no));
                if (mBinding.checkboxAdductor.isChecked())
                    jsonObject.put(S.lumbar_epidural, getString(R.string.yes));
                else
                    jsonObject.put(S.lumbar_epidural, getString(R.string.no));
                if (mBinding.checkboxPlexus.isChecked())
                    jsonObject.put(S.spinal_single_shot, getString(R.string.yes));
                else
                    jsonObject.put(S.spinal_single_shot, getString(R.string.no));

                if (mBinding.checkboxSciatic.isChecked())
                    jsonObject.put(S.caudal_single_shot, getString(R.string.yes));
                else
                    jsonObject.put(S.caudal_single_shot, getString(R.string.no));

                if (mBinding.checkboxCandel.isChecked())
                    jsonObject.put(S.continuous_caudal, getString(R.string.yes));
                else
                    jsonObject.put(S.continuous_caudal, getString(R.string.no));
    ...
    private void autofillInvasiveLine(@NonNull CustomArrayList<AdvancedQIModal> arrayList) {
        if (arrayList.contains(S.thoracic_epidural.toLowerCase())) {
            int pos = arrayList.indexOf(S.thoracic_epidural.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
                mBinding.checkboxFemoral.setChecked(true);
        }
        if (arrayList.contains(S.lumbar_epidural.toLowerCase())) {
            int pos = arrayList.indexOf(S.lumbar_epidural.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
                mBinding.checkboxAdductor.setChecked(true);
        }
        if (arrayList.contains(S.spinal_single_shot.toLowerCase())) {
            int pos = arrayList.indexOf(S.spinal_single_shot.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
                mBinding.checkboxPlexus.setChecked(true);
        }
        if (arrayList.contains(S.caudal_single_shot.toLowerCase())) {
            int pos = arrayList.indexOf(S.caudal_single_shot.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
                mBinding.checkboxSciatic.setChecked(true);
        }
        if (arrayList.contains(S.continuous_caudal.toLowerCase())) {
            int pos = arrayList.indexOf(S.continuous_caudal.toLowerCase());
            if (arrayList.get(pos).getId().equalsIgnoreCase(getString(R.string.yes)))
