package com.octalsoftaware.archi.lib.swipe;
...
public class SwipeLayout extends FrameLayout {
    ...
    public SwipeLayout(@NonNull Context context, AttributeSet attrs, int defStyle) {
     ...
     */
    public void addRevealListener(@NonNull int[] childIds, OnRevealListener l) {
        for (int i : childIds)
     ...
     */
    protected boolean isViewTotallyFirstShowed(View child, @NonNull Rect relativePosition, DragEdge edge, int surfaceLeft,
    ...
    protected boolean isViewShowing(View child, @NonNull Rect relativePosition, @NonNull DragEdge availableEdge, int surfaceLeft,
                                    int surfaceTop, int surfaceRight, int surfaceBottom) {
        int childLeft = relativePosition.left;
        int childRight = relativePosition.right;
        int childTop = relativePosition.top;
    ...
    @NonNull
    protected Rect getRelativePosition(@NonNull View child) {
        View t = child;
    ...
    public void setDrag(@NonNull DragEdge dragEdge, int childId) {
        clearDragEdge();
        addDrag(dragEdge, childId);
    }

    public void setDrag(@NonNull DragEdge dragEdge, View child) {
        clearDragEdge();
        addDrag(dragEdge, child);
    }

    public void addDrag(@NonNull DragEdge dragEdge, int childId) {
        addDrag(dragEdge, findViewById(childId), null);
    }

    public void addDrag(@NonNull DragEdge dragEdge, View child) {
        addDrag(dragEdge, child, null);
    }

    public void addDrag(@NonNull DragEdge dragEdge, @Nullable View child, @Nullable ViewGroup.LayoutParams params) {
        if (child == null) return;
    ...
    @Override
    public void addView(@Nullable View child, int index, @NonNull ViewGroup.LayoutParams params) {
        if (child == null) return;
    ...
    private void checkCanDrag(@NonNull MotionEvent ev) {
    ...
    private boolean isTouchOnSurface(@NonNull MotionEvent ev) {
        View surfaceView = getSurfaceView();
        if (surfaceView == null) {
            return false;
        }
     ...
     */
    @Deprecated
    public void setDragEdges(@NonNull List<DragEdge> dragEdges) {
        clearDragEdge();
